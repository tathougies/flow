...
import Language.Flow.Lambda.Typed
...
compileProgram options program = liftM fst $ runStateT action (emptyState {options = options})
    where
      action = do
        ...
        liftIO $ infoM moduleName $ L.prettyPrintAST $ simplifiedPatternsBody

        -- Now we have to convert lets into letrecs and simple lets...
        let body' = liftSimpleLets simplifiedPatternsBody

        liftIO $ infoM moduleName "\nAfter making simple lets:"
        liftIO $ infoM moduleName $ L.prettyPrintAST body'
        let te = TypeEnv M.empty
        case typeCheck te nameSequence body' of
          Left e -> case e of
                          CantConstructInfiniteType r v t -> compileError r $
                                                             concat ["Can't construct infinite type: ", show v, " = ", show t]
                          CouldNotMatchTypes r t1 t2 -> compileError r $
                                                        concat ["Could not match types. Expected: ", show t1, ". Actual: ", show t2, "."]
                          UnknownError s -> error . concat $ ["Type check internal error: ", show s]
          Right typ -> liftIO $ infoM moduleName $ concat ["Expression type: ", show typ]
        ...
        liftIO $ infoM moduleName $ show program
        ...
        return $ gcodeProgram { initialData = initData ++ initialData gcodeProgram,
                                ...
                                progDebug = startInDebug options || startTracing options,
                                progTrace = startTracing options}
...
resolveIdentifiers expr = resolveIdentifiers' M.empty expr
    where
      ...
      resolveIdentifiers' scope (L.SimpleLet _ _ _) = error "Simple lets should not be generated by parser"
...
simplifyFunctionPatterns (L.Lambda r patterns expression) = do
  ...
  return $ L.Lambda r patterns e'
simplifyFunctionPatterns (L.SimpleLet _ _ _) = error "Simple lets should only be used internally"
...
appearsFreeIn :: VarID -> L.Expression -> Bool
appearsFreeIn _ (L.Identifier _ _) = False
appearsFreeIn _ (L.Literal _ _) = False
appearsFreeIn var (L.Ap _ e1 e2) = (var `appearsFreeIn` e1) || (var `appearsFreeIn` e2)
appearsFreeIn var (L.LetIn _ _ bindings e)
    -- If the variable is rebound in the let, then it's not free in the let
    | any (\(L.Binding _ (L.VarIDPattern _ v) _) -> v == var) bindings = False
    | otherwise = any (\(L.Binding _ _ be) -> var `appearsFreeIn` be) bindings ||
                  var `appearsFreeIn` e
appearsFreeIn var (L.SimpleLet _ (v, b) e)
    | var == v = False
    | otherwise = var `appearsFreeIn` b || var `appearsFreeIn` e
appearsFreeIn var (L.TypeAssertionE _ _ _) = error "can't determine if things appear free in type assertion"
appearsFreeIn var (L.Block _ es) = any (var `appearsFreeIn`) es
appearsFreeIn var (L.Lambda _ vs e)
    | any (\(L.VarIDPattern _ v) -> v == var) vs = False
    | otherwise = var `appearsFreeIn` e
appearsFreeIn var (L.Case _ expr patsAndExprs) = (var `appearsFreeIn` expr) || inPatsAndExprs
    where inPatsAndExprs = any (uncurry inPatAndExpr) patsAndExprs
          inPatAndExpr pat expr
              -- If the variable appears in the pattern, then it's not free in the expression
              | any (==var) (boundPatVariables pat) = False
              | otherwise = var `appearsFreeIn` expr
appearsFreeIn _ (L.LocationRef _ _) = False
appearsFreeIn var (L.VariableRef _ var1)
    | var == var1 = True
    | otherwise   = False
...
lowerASTIntoEnrichedLambdaCalculus (L.LetIn _ [] [L.Binding _ (L.VarIDPattern _ v) e] expression)
    -- This can usually be converted into a simple let, unless we're doing recursion...
    | v `appearsFreeIn` e = E.Let [(v, lowerASTIntoEnrichedLambdaCalculus e)] (lowerASTIntoEnrichedLambdaCalculus expression)
    | otherwise = E.SimpleLet (v, lowerASTIntoEnrichedLambdaCalculus e) $ lowerASTIntoEnrichedLambdaCalculus expression
...
lowerASTIntoEnrichedLambdaCalculus (L.SimpleLet _ (v, b) e) = E.SimpleLet (v, lowerASTIntoEnrichedLambdaCalculus b) (lowerASTIntoEnrichedLambdaCalculus e)
...
boundPatVariables :: L.Pattern -> [VarID]
boundPatVariables (L.VariablePattern _ _) = error "Found variable name, expecting ID"
boundPatVariables (L.VarIDPattern _ i) = [i]
boundPatVariables (L.PlaceholderPattern _) = []
boundPatVariables (L.ConstrPattern _ _ pats) = concatMap boundPatVariables pats

freeVars :: L.Expression -> S.Set VarID
freeVars e = snd (runWriter (collectFreeVars S.empty e))
    where collectFreeVars :: S.Set VarID -> L.Expression -> Writer (S.Set VarID) ()
          collectFreeVars bound (L.Identifier _ _) = return ()
          collectFreeVars bound (L.Literal _ _) = return ()
          collectFreeVars bound (L.LocationRef _ _) = return ()
          collectFreeVars bound (L.VariableRef _ v)
              | v `S.member` bound = return ()
              | otherwise = tell (S.singleton v)
          collectFreeVars bound (L.Ap _ e1 e2) = collectFreeVars bound e1 >>
                                                 collectFreeVars bound e2
          collectFreeVars bound (L.LetIn _ _ bindings e) =
              let rebound = S.fromList $ map (\(L.Binding _ (L.VarIDPattern _ v) _) -> v) bindings
                  bound' = bound `S.union` rebound
              in mapM_ (\(L.Binding _ _ e) -> collectFreeVars bound' e) bindings >> collectFreeVars bound' e
          collectFreeVars bound (L.SimpleLet _ (v, b) e) =
              let bound' = bound `S.union` S.singleton v
              in collectFreeVars bound b >> -- No recursion for simple lets
                 collectFreeVars bound' e
          collectFreeVars bound (L.TypeAssertionE _ _ _) = return ()
          collectFreeVars bound (L.Block _ exprs) = mapM_ (collectFreeVars bound) exprs
          collectFreeVars bound (L.Lambda _ [L.VarIDPattern _ v] e) =
              let bound' = bound `S.union` S.singleton v
              in collectFreeVars bound' e
          collectFreeVars bound (L.Case _ e patsAndExprs) = collectFreeVars bound e >>
                                                            (forM_ patsAndExprs $ \(pat, expr) ->
                                                                 let bound' = bound `S.union` (S.fromList (boundPatVariables pat))
                                                                 in collectFreeVars bound' expr)

liftSimpleLets :: L.Expression -> L.Expression
liftSimpleLets x@(L.Identifier _ _) = x
liftSimpleLets x@(L.Literal _ _) = x
liftSimpleLets x@(L.VariableRef _ _) = x
liftSimpleLets x@(L.LocationRef _ _) = x
liftSimpleLets x@(L.SimpleLet _ _ _) = x
liftSimpleLets (L.Ap r e1 e2) = L.Ap r (liftSimpleLets e1) (liftSimpleLets e2)
liftSimpleLets (L.Block r exprs) = L.Block r (map liftSimpleLets exprs)
liftSimpleLets (L.Lambda r pat expr) = L.Lambda r pat (liftSimpleLets expr)
liftSimpleLets (L.Case r e patsAndExprs) = L.Case r (liftSimpleLets e) (map (\(pat, expr) -> (pat, liftSimpleLets expr)) patsAndExprs)
liftSimpleLets (L.LetIn r typeAs bindings expr) =
    let bindings' = map (\(L.Binding _ (L.VarIDPattern _ v) e) -> (v, liftSimpleLets e)) bindings
        boundVars = map fst bindings'
        boundVarsSet = S.fromList boundVars

        usedVarss = map (S.toList . (boundVarsSet `S.intersection`) . freeVars . snd) bindings'

        bindingGraph = zip3 bindings' boundVars usedVarss

        sccs = stronglyConnComp bindingGraph

        outerLet = foldl (\e scc -> case scc of
                                      AcyclicSCC (v, b) -- Sometimes acyclic nodes aren't really simple lets, because they're auto-recursive
                                          | v `appearsFreeIn` b -> L.LetIn L.EmptyRegion [] [L.Binding L.EmptyRegion (L.VarIDPattern L.EmptyRegion v) b] e
                                          | otherwise -> L.SimpleLet L.EmptyRegion (v, b) e
                                      CyclicSCC vbs -> L.LetIn L.EmptyRegion []
                                                       (map (\(v, b) -> L.Binding L.EmptyRegion (L.VarIDPattern L.EmptyRegion v) b) vbs)
                                                       e) expr (reverse sccs)
    in outerLet
